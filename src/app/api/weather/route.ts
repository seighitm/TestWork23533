...
export async function GET(req: Request) {
  ...
  try {
    const weatherResponse = await axios.get(
      `${OPENWEATHERMAP_BASE_URL}data/2.5/weather?q=${city},${country}&appid=${process.env.OPENWEATHERMAP_API_KEY}&units=metric`,
    );

    return new Response(JSON.stringify(weatherResponse.data), {
      status: 200,
      statusText: 'OK',
    });
  } catch (err) {
